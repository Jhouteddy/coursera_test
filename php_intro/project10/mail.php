...
if (isset($_POST["submit"])) {
    ...
    if (empty($name) || empty($email) || empty($pass1) || empty($pass2) || empty($message)) {
        echo "<sapn class='from-error'>請輸入完整訊息</span>";
        $errorEmpty = true;
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        echo "<sapn class='from-error'>請輸入正確email地址</span>";
        $errorEmail = true;
    } elseif ($pass1 != $pass2) {
        echo "<sapn class='from-error'>請輸入相同的密碼</span>";
        $errorPass = true;
    } else {
        $mailToname = "a0936695551@gmail.com";
        $mailTo = "a0936695551@gmail.com";
        $mailFromname = $name;
        $mailFrom = $email;
        $mailSubject = "網站聯繫表單";
        $mailContent = "
        ...
        if (mail($mailTo, $mailSubject, $mailContent, $mailFrom)) {
            echo "<sapn class='from-success'>郵件發送成功!</span>";
        } else{
            echo "<sapn class='from-error'>郵件發送失敗!</span>";
        }
    }
} else {
    echo "<sapn class='from-error'>網路錯誤,稍後再試試看!</span>";
}
?>
